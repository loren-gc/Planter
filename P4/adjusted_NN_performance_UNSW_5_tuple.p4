...
control SwitchIngress(
    ...
    action Layer0_Process1(bit <10> offset){
        ...
        meta.current_reg_index = (bit<32>)offset+1;
        weights.read(weight, meta.current_reg_index);
        XNOR(weight);
        meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
        BitCount_l1(meta.XNOROutput);
        meta.current_reg_index = (bit<32>)offset+2;
        weights.read(weight, meta.current_reg_index);
        XNOR(weight);
        meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
        BitCount_l1(meta.XNOROutput);
        meta.current_reg_index = (bit<32>)offset+3;
        weights.read(weight, meta.current_reg_index);
        XNOR(weight);
        meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
        BitCount_l1(meta.XNOROutput);
        meta.current_reg_index = (bit<32>)offset+4;
        weights.read(weight, meta.current_reg_index);
        XNOR(weight);
        meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
        BitCount_l1(meta.XNOROutput);
        meta.current_reg_index = (bit<32>)offset+5;
        weights.read(weight, meta.current_reg_index);
        XNOR(weight);
        meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
        BitCount_l1(meta.XNOROutput);
        meta.current_reg_index = (bit<32>)offset+6;
        weights.read(weight, meta.current_reg_index);
        XNOR(weight);
        meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
        BitCount_l1(meta.XNOROutput);
        meta.current_reg_index = (bit<32>)offset+7;
        weights.read(weight, meta.current_reg_index);
        XNOR(weight);
        meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
        BitCount_l1(meta.XNOROutput);
        meta.current_reg_index = (bit<32>)offset+8;
        weights.read(weight, meta.current_reg_index);
        XNOR(weight);
        meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
        BitCount_l1(meta.XNOROutput);
        meta.current_reg_index = (bit<32>)offset+9;
        weights.read(weight, meta.current_reg_index);
        XNOR(weight);
        meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
        BitCount_l1(meta.XNOROutput);
        meta.current_reg_index = (bit<32>)offset+10;
        weights.read(weight, meta.current_reg_index);
        XNOR(weight);
        meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
        BitCount_l1(meta.XNOROutput);
        meta.current_reg_index = (bit<32>)offset+11;
        weights.read(weight, meta.current_reg_index);
        XNOR(weight);
        meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
        BitCount_l1(meta.XNOROutput);
        meta.current_reg_index = (bit<32>)offset+12;
        weights.read(weight, meta.current_reg_index);
        XNOR(weight);
        meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
        BitCount_l1(meta.XNOROutput);
        meta.current_reg_index = (bit<32>)offset+13;
        weights.read(weight, meta.current_reg_index);
        XNOR(weight);
        meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
        BitCount_l1(meta.XNOROutput);
        meta.current_reg_index = (bit<32>)offset+14;
        weights.read(weight, meta.current_reg_index);
        XNOR(weight);
        meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
        BitCount_l1(meta.XNOROutput);
        meta.current_reg_index = (bit<32>)offset+15;
        weights.read(weight, meta.current_reg_index);
        XNOR(weight);
        meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
        BitCount_l1(meta.XNOROutput);
        meta.current_reg_index = (bit<32>)offset+16;
        weights.read(weight, meta.current_reg_index);
        XNOR(weight);
        meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
        BitCount_l1(meta.XNOROutput);
        meta.current_reg_index = (bit<32>)offset+17;
        weights.read(weight, meta.current_reg_index);
        XNOR(weight);
        meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
        BitCount_l1(meta.XNOROutput);
        meta.current_reg_index = (bit<32>)offset+18;
        weights.read(weight, meta.current_reg_index);
        XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	...
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+20;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+21;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+22;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+23;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+24;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+25;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+26;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+27;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+28;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+29;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+30;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+31;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+32;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+33;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+34;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+35;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+36;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+37;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+38;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+39;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+40;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+41;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+42;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+43;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+44;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+45;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+46;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+47;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+48;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+49;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+50;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+51;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+52;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+53;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+54;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
	meta.current_reg_index = (bit<32>)offset+55;
	weights2.read(weight, meta.current_reg_index);
	XNOR(weight);
	meta.XNOROutput = (bit<64>)meta.XNOROutput[55:0];
	BitCount_l1(meta.XNOROutput);
